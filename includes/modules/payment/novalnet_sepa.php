...
class novalnet_sepa extends NovalnetPayment
{
    ...
    function __construct()
    {
        ...
        if (strpos(MODULE_PAYMENT_INSTALLED, $this->code) !== false) {
            if(defined('MODULE_PAYMENT_NOVALNET_SEPA_ENABLE_FRAUDMODULE')){
    ...
    public function selection()
    {
        ...
        <input type="hidden" name="nn_sepa_account_error" id="nn_sepa_account_error" value="'.MODULE_PAYMENT_NOVALNET_VALID_ACCOUNT_CREDENTIALS_ERROR.'">';
        ...
        if (isset($_SESSION['novalnet'][$this->code]['tid']) && $this->fraud_module && $this->fraud_module_status) {
            ...
            if ($payment_implementation_type == 'guarantee' && empty($order->billing['company'])) {
			...
			$bic_field  = '<tr id="novalnet_sepa_bic_tr" style="display:none;" > <td class = "main"> BIC <span style="color:red">*</span> </td> <td class = "main">'. xtc_draw_input_field('novalnet_sepa_bic', '', 'id="novalnet_sepa_bic" autocomplete="off" style="text-transform:uppercase" onkeypress="return NovalnetUtility.formatBic(event);" onchange="return NovalnetUtility.formatBic(event);"') . '</td> </tr>';
            $selection['fields'][] = array(
                    'title' => '',
                    'field' => '<div id="nn_sepa_acc" style="display:none">
                             <table>'. NovalnetUtil::showMaskedDetails(array(
											array(
                                                ...
                                                'value' => xtc_draw_input_field('novalnet_sepa_iban', '', 'id="novalnet_sepa_iban" autocomplete="off" style="text-transform:uppercase" onkeypress="return NovalnetUtility.checkIban(event, \'novalnet_sepa_bic_tr\');" onkeyup="return NovalnetUtility.formatIban(event, \'novalnet_sepa_bic_tr\');" onchange="return NovalnetUtility.formatIban(event, \'novalnet_sepa_bic_tr\');"'),
                                                'required' => true,
                                            ),
                                        )) . $bic_field . $guarantee_field . $sepa_fields . $pin_by_callback . '</table></div>');
            if (MODULE_PAYMENT_NOVALNET_SEPA_SHOP_TYPE == 'ONECLICK') {
                  $selection['fields'][] = array(
                       'title' => '',
    ...
    public function pre_confirmation_check()
    {
        ...
        list($payment_implementation_type, $error) = NovalnetUtil::paymentImplementationType($order, $this->code);
        if ($payment_implementation_type == 'error') {
			$payment_error_return = 'payment_error=' . $this->code . '&error=' . urlencode(NovalnetUtil::setUtf8Mode(filter_var($error, FILTER_SANITIZE_STRING)));
        ...
        if ($error_message != '' && MODULE_PAYMENT_NOVALNET_SEPA_FORCE_NON_GUARANTEE == 'False'){
            ...
            $payment_error = 'payment_error=' . $this->code . '&error=' . urlencode($error_message);
			xtc_redirect(xtc_href_link(FILENAME_CHECKOUT_PAYMENT, $payment_error, 'SSL', true, false));
        }
        ...
        if (isset($_SESSION['novalnet'][$this->code]['secondcall'])) {
            if ($_SESSION['novalnet'][$this->code]['order_amount'] != NovalnetUtil::getPaymentAmount((array)$order)) {
                if (isset($_SESSION['novalnet'])) {
					...
					$payment_error_return = 'payment_error=' . $this->code . '&error=' . urlencode(NovalnetUtil::setUtf8Mode(MODULE_PAYMENT_NOVALNET_FRAUDMODULE_AMOUNT_CHANGE_ERROR));
					xtc_redirect(xtc_href_link(FILENAME_CHECKOUT_PAYMENT, $payment_error_return, 'SSL', true, false));
            }
        }
    ...
    public function process_button()
    {
        ...
        if (isset($post[$this->code . '_new_pin']) && $post[$this->code . '_new_pin'] == 1) {
			...
			$payment_error_return = 'payment_error=' . $this->code . '&error=' . urlencode($response['status_message']);
            xtc_redirect(xtc_href_link(FILENAME_CHECKOUT_PAYMENT, 'error_message=' . $payment_error_return, 'SSL', true, false));
          ...
          // Display error message
            $payment_error_return = 'error_message=' .urlencode(MODULE_PAYMENT_NOVALNET_AMOUNT_ERROR_MESSAGE);
            xtc_redirect(xtc_href_link(FILENAME_CHECKOUT_PAYMENT, $payment_error_return, 'SSL', true, false));
        }
    ...
    public function before_process()
    {
        ...
        if (isset($param_inputs['secondcall']) && $param_inputs['secondcall']) {
            ...
            if ($response['status'] != 100) {
				...
				$payment_error_return = 'payment_error=' . $this->code . '&error=' . urlencode(!empty($response['status_message']) ? NovalnetUtil::setUtf8Mode($response['status_message']) : $response['pin_status']['status_message']);
				xtc_redirect(xtc_href_link(FILENAME_CHECKOUT_PAYMENT, $payment_error_return, 'SSL', true, false));
            }
            ...
            if (!empty($_SESSION['novalnet'][$this->code]['nn_payment_ref_tid_sepa']) && $_SESSION['novalnet'][$this->code]['novalnet_sepa_savecard'] == '1' && $_SESSION['novalnet'][$this->code]['novalnet_sepa_savecheck'] == 1) {
                ...
                $input_params['bic'] = $_SESSION['novalnet'][$this->code]['novalnet_sepa_bic'];
            }
            ...
            if($payment_implementation_type == 'guarantee' && (!$invalid_valid_dob || $order->billing['company'] != '')) {
                ...
                if(!empty($_SESSION['novalnet'][$this->code]['novalnet_sepa_birthdate_final'])) {
                    $input_params['birth_date']   = date('Y-m-d', strtotime($_SESSION['novalnet'][$this->code]['novalnet_sepa_birthdate_final']));
                } else {
            ...
            if ($payment_response['status'] == '100') {
                ...
                // Novalnet transaction status got failure for displaying error message
                $payment_error_return = 'payment_error=' . $this->code . '&error=' . urlencode(NovalnetUtil::getTransactionMessage($payment_response));
				xtc_redirect(xtc_href_link(FILENAME_CHECKOUT_PAYMENT, $payment_error_return, 'SSL', true, false));
            }
        }
        ...
        if(MODULE_PAYMENT_NOVALNET_SEPA_SHOP_TYPE == 'ZEROAMOUNT'){
            $transaction_comments .= PHP_EOL.MODULE_PAYMENT_NOVALNET_ZERO_AMOUNT_BOOKING_MESSAGE;
        }
        // Update Novalnet transaction comments
    ...
    public function install()
    {
        if (NovalnetUtil::globalConfigInstallError($this->code)) {
            ...
            ('MODULE_PAYMENT_NOVALNET_SEPA_STATUS', 'False',  '6', '1', 'xtc_cfg_select_option(array(\'True\', \'False\'), ','', now()),
            ('MODULE_PAYMENT_NOVALNET_SEPA_TEST_MODE', 'False',  '6', '2', 'xtc_cfg_select_option(array(\'True\', \'False\'), ','', now()),
            ('MODULE_PAYMENT_NOVALNET_SEPA_CAPTURE_AUTHORIZE', 'capture',  '6', '3', 'xtc_cfg_select_option(array(\'capture\', \'authorize\'), ','', now()),
            ('MODULE_PAYMENT_NOVALNET_SEPA_MANUAL_CHECK_LIMIT', '', '6', '4', '', '', now()),
            ('MODULE_PAYMENT_NOVALNET_SEPA_ENABLE_FRAUDMODULE', 'False',  '6', '5', 'xtc_cfg_select_option(array(\'False\', \'CALLBACK\',\'SMS\'), ','', now()),
            ('MODULE_PAYMENT_NOVALNET_SEPA_CALLBACK_LIMIT', '', '6', '6','',  '', now()),
            ('MODULE_PAYMENT_NOVALNET_SEPA_PAYMENT_DUE_DATE', '', '6', '7','',  '', now()),
            ('MODULE_PAYMENT_NOVALNET_SEPA_SHOP_TYPE', 'False',  '6', '8', 'xtc_cfg_select_option(array(\'False\', \'ONECLICK\',\'ZEROAMOUNT\'), ','', now()),
            ...
            ('MODULE_PAYMENT_NOVALNET_SEPA_ENABLE_GUARANTEE', 'False',  '6', '14', 'xtc_cfg_select_option(array(\'True\', \'False\'), ','', now()),
            ...
            ('MODULE_PAYMENT_NOVALNET_SEPA_FORCE_NON_GUARANTEE', 'True',  '6', '17', 'xtc_cfg_select_option(array(\'True\', \'False\'), ','', now())
            ");
        }
    }
    ...
    {
        if(defined(MODULE_PAYMENT_NOVALNET_SEPA_STATUS)){
