...
class novalnet_sepa extends NovalnetPayment
{
    ...
    public function selection()
    {
        ...
        if (MODULE_PAYMENT_NOVALNET_SEPA_SHOP_TYPE == 'ZEROAMOUNT' && $this->fraud_module_status != '1' && ( $payment_implementation_type == 'normal' || !$payment_implementation_type)) {
            $notification .=  MODULE_PAYMENT_NOVALNET_SEPA_ZERO_AMOUNT_COMMENTS;
        }
        ...
        <script src="'.DIR_WS_CATALOG.'includes/external/novalnet/js/novalnet_sepa.js'.'" type="text/javascript"></script>
        <noscript>
        <input type="hidden" name="nn_sepa_js_enabled" value="1"><br />
        <div style="color:red"><b>'. MODULE_PAYMENT_NOVALNET_JS_DEACTIVATE_ERROR . '</b></div></noscript>';
        ...
        if (isset($_SESSION['novalnet'][$this->code]['tid']) && $this->fraud_module && $this->fraud_module_status) {
                 ...
                 NovalnetUtil::showAlternateShoppingType('novalnet_oneclick_sepa_ref', MODULE_PAYMENT_NOVALNET_ONECLICK_SEPA_REF).'
                    <table class="paymentmoduledata" id="novalnet_oneclick_sepa_ref_div">'.
                        NovalnetUtil::showMaskedDetails(array(
                            array(
                                'label' => MODULE_PAYMENT_NOVALNET_ONECLICK_SEPA_ACC,
            ...
            $selection['fields'][] = array(
                    ...
                    'field' => '<div id="nn_sepa_acc" style="display:none">
                             <table>'. NovalnetUtil::showMaskedDetails(array(
                                            array(
                                                'label' => MODULE_PAYMENT_NOVALNET_ACCOUNT_HOLDER,
                                                'value' => xtc_draw_input_field('novalnet_sepa_account_holder', ($_SESSION['acc_holder_details'] == '') ? $customer_name : $_SESSION['acc_holder_details'], 'id="novalnet_sepa_account_holder" autocomplete="off" onkeypress="return iban_validate(event, true);"'),
                                                'required' => true,
                                            ),
                                            array(
            ...
            if (MODULE_PAYMENT_NOVALNET_SEPA_SHOP_TYPE == 'ONECLICK') {
                  $selection['fields'][] = array(
                       'title' => '',
                       'field' => '<div id="nn_sepa_savecard" style="display:none;">'.xtc_draw_checkbox_field('nn_sepa_savecard', 1, false, 'id=nn_sepa_savecard').MODULE_PAYMENT_NOVALNET_SEPA_SAVECARD_DETAILS
                                   );
            }
                $selection['fields'][] =  array(
                    'title' => '',
                    'field' => '<div id="nn_sepa_mandate" style="display:none;"><table>'. NovalnetUtil::showMaskedDetails(array(
                                            array(
                                                'label' => '',
                                                'value' => MODULE_PAYMENT_NOVALNET_SEPA_FORM_MANDATE_CONFIRM_TEXT,
                                            ))).'</table></div>');
        }
    ...
    public function pre_confirmation_check()
    {
        ...
        if (!empty($post['nn_sepa_js_enabled'])) {
            xtc_redirect(xtc_href_link(FILENAME_CHECKOUT_PAYMENT, 'error_message=' . MODULE_PAYMENT_NOVALNET_JS_DEACTIVATE_ERROR, 'SSL', true, false));
        }
        ...
        if ($payment_implementation_type == 'guarantee') {

            $_SESSION['novalnet'][$this->code]['process_guarantee'] = true;

            if (MODULE_PAYMENT_NOVALNET_SEPA_FORCE_NON_GUARANTEE != 'True' && empty ($order->customer['company']) ) {
				
                // Validate age
                if (empty($_SESSION['novalnet'][$this->code]['novalnet_sepa_birthdate_final'])) {                   
                    xtc_redirect(xtc_href_link(FILENAME_CHECKOUT_PAYMENT, 'error_message=' . NovalnetUtil::setUtf8Mode(MODULE_PAYMENT_NOVALNET_GUARANTEE_DOB_EMPTY_ERROR), 'SSL', true, false));
                } elseif (NovalnetUtil::validateAge($_SESSION['novalnet'][$this->code]['novalnet_sepa_birthdate_final'])) { 
					xtc_redirect(xtc_href_link(FILENAME_CHECKOUT_PAYMENT, 'error_message=' . NovalnetUtil::setUtf8Mode(MODULE_PAYMENT_NOVALNET_AGE_ERROR), 'SSL', true, false));
                }
            } 
        }      
    }
    ...
    public function before_process()
    {
        ...
        if (isset($param_inputs['secondcall']) && $param_inputs['secondcall']) {
            ...
            if ((MODULE_PAYMENT_NOVALNET_SEPA_CAPTURE_AUTHORIZE == 'authorize' && $_SESSION['novalnet'][$this->code]['order_amount'] >= MODULE_PAYMENT_NOVALNET_SEPA_MANUAL_CHECK_LIMIT && (MODULE_PAYMENT_NOVALNET_SEPA_SHOP_TYPE != 'ZEROAMOUNT'))) {
            ...
            if (!empty($_SESSION['novalnet'][$this->code]['nn_payment_ref_tid_sepa']) && $_SESSION['novalnet'][$this->code]['novalnet_sepa_savecard'] == '1' && $_SESSION['novalnet'][$this->code]['novalnet_sepa_savecheck'] == 1) {
                ...
                if ((MODULE_PAYMENT_NOVALNET_SEPA_SHOP_TYPE == 'ONECLICK' && $_SESSION['nn_sepa_savecard'] == 1) || (MODULE_PAYMENT_NOVALNET_SEPA_SHOP_TYPE == 'ZEROAMOUNT' && $_SESSION['novalnet'][$this->code]['process_guarantee'] != 1)) {
                    $input_params['create_payment_ref'] = '1';
                }

                $input_params['bank_account_holder'] = $_SESSION['novalnet'][$this->code]['novalnet_sepa_account_holder'];
            ...
            if (!empty($_SESSION['novalnet'][$this->code]['process_guarantee']) && $payment_implementation_type == 'guarantee') {
                // Appending parameters for guarantee payment
                $input_params['key']          = 40;
                $input_params['payment_type'] = 'GUARANTEED_DIRECT_DEBIT_SEPA';
                if( empty ($order->customer['company'])  && !empty($_SESSION['novalnet'][$this->code]['novalnet_sepa_birthdate_final'])) {
                    $input_params['birth_date']   = $_SESSION['novalnet'][$this->code]['novalnet_sepa_birthdate_final'];
                }
            } else {
    ...
    public function after_process()
    {
        ...
        if ($_SESSION['novalnet'][$this->code]['gateway_status'] == '100') {
            $order_status = MODULE_PAYMENT_NOVALNET_SEPA_ORDER_STATUS;
        } else {
            $order_status = ($_SESSION['novalnet'][$this->code]['process_guarantee'] == '1') ? ($_SESSION['novalnet'][$this->code]['gateway_status'] == '75' ? MODULE_PAYMENT_NOVALNET_SEPA_GUARANTEE_PENDING_ORDER_STATUS : MODULE_PAYMENT_NOVALNET_ONHOLD_ORDER_COMPLETE) : MODULE_PAYMENT_NOVALNET_ONHOLD_ORDER_COMPLETE;
        }
    ...
    public function install()
    {
        if (NovalnetUtil::globalConfigInstallError($this->code)) {
            ...
            ('MODULE_PAYMENT_NOVALNET_SEPA_ORDER_STATUS', '1',  '6', '12', 'xtc_cfg_pull_down_order_statuses(', 'xtc_get_order_status_name', now()),
            ...
            ('MODULE_PAYMENT_NOVALNET_SEPA_GUARANTEE_MIN_AMOUNT', '', '6', '15','',  '', now()),
            ('MODULE_PAYMENT_NOVALNET_SEPA_FORCE_NON_GUARANTEE','True', '6', '16', 'xtc_mod_select_option(array(\'True\' => MODULE_PAYMENT_NOVALNET_TRUE,\'False\' => MODULE_PAYMENT_NOVALNET_FALSE),\'MODULE_PAYMENT_NOVALNET_SEPA_FORCE_NON_GUARANTEE\',".MODULE_PAYMENT_NOVALNET_SEPA_FORCE_NON_GUARANTEE.",' , '',now()),
            ('MODULE_PAYMENT_NOVALNET_SEPA_GUARANTEE_PENDING_ORDER_STATUS', '1',  '6', '17', 'xtc_cfg_pull_down_order_statuses(', 'xtc_get_order_status_name', now())
            ");
        }
    }
