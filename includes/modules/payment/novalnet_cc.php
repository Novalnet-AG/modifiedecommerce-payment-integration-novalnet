...
class novalnet_cc extends NovalnetPayment
{
    ...
    function __construct()
    {
        ...
        $this->setPaymentDetails(); 
         if (strpos(MODULE_PAYMENT_INSTALLED, $this->code) !== false) {
			if (isset($_SESSION['novalnet'][$this->code]['nn_do_redirect']) && $_SESSION['novalnet'][$this->code]['nn_do_redirect'] == '1') {
                $this->tmpOrders = 1;
    ...
    public function pre_confirmation_check()
    {
        ...
        if ($post['novalnet_cc_newcard'] == 'new' && (empty($post['nn_cc_pan_hash']) || empty($post['nn_cc_uniqueid']))) {
			$payment_error_return = 'payment_error=' . $this->code . '&error=' . urlencode(NovalnetUtil::setUtf8Mode(MODULE_PAYMENT_NOVALNET_VALID_CC_DETAILS));
  ...
  /**
    ...
    public function before_process()
    {
		...
		if (isset($post['tid'])) { // 3D secure
			if (NovalnetUtil::validateHashResponse($post)) { // Validating the hash for 3D secure transaction
			...
			$_SESSION['novalnet'][$this->code] = array_merge($this->redirectInitialParams($payment_response), $this->paymentInitialParams($payment_response), array(
                ...
                'nn_cc_savecard'        => $_SESSION['novalnet'][$this->code]['nn_cc_savecard'],
        ...
        if (isset($payment_response['status'])) { // If 3D or non 3D transaction response received
            if($payment_response['status'] == '100') { // Handling success response
                // Set order status
            ...
            else { // Handling failure response
    ...
    {
        global $order;
        $inputparams= $this->formParameters();

        if (constant('MODULE_PAYMENT_'.strtoupper($this->code).'_SHOP_TYPE') == 'ZEROAMOUNT') {
            unset($inputparams['tariff_type'], $inputparams['on_hold']);
            // Assigning parameter for zero amount booking
            $_SESSION['novalnet'][$this->code]['order_amount']=0;
            $_SESSION['novalnet'][$this->code]['zero_transaction'] = 1;
            $_SESSION['novalnet'][$this->code]['zerotrxndetails'] = serialize($inputparams);
        ...
        if($inputparams['create_payment_ref']){
            $redirect_data['create_payment_ref']=$inputparams['create_payment_ref'];
        }
        if($inputparams['pan_hash']){
            $redirect_data['pan_hash']=$inputparams['pan_hash'];
        }
        if($inputparams['unique_id']){
            $redirect_data['unique_id']=$inputparams['unique_id'];
        }
        if($inputparams['on_hold']){
            $redirect_data['on_hold']=$inputparams['on_hold'];
        }
        if($inputparams['nn_it']){
            $redirect_data['nn_it']=$inputparams['nn_it'];
        }
        ...
        if(MODULE_PAYMENT_NOVALNET_CC_ENFORCE_3D == 'True'){
            $redirect_data['enforce_3d']=1;  
        }
   ...
   /**
    ...
    public function install()
    {
        if (NovalnetUtil::globalConfigInstallError($this->code)) {
            ...
            ('MODULE_PAYMENT_NOVALNET_CC_STATUS', 'False',  '6', '1', 'xtc_cfg_select_option(array(\'True\', \'False\'), ','', now()),
            ('MODULE_PAYMENT_NOVALNET_CC_TEST_MODE', 'False',  '6', '2', 'xtc_cfg_select_option(array(\'True\', \'False\'), ','', now()),
            ('MODULE_PAYMENT_NOVALNET_CC_ENFORCE_3D', 'False',  '6', '2', 'xtc_cfg_select_option(array(\'True\', \'False\'), ','', now()),
            ('MODULE_PAYMENT_NOVALNET_CC_INLINE_FORM', 'True',  '6', '3', 'xtc_cfg_select_option(array(\'True\', \'False\'), ','', now()),
            
            ('MODULE_PAYMENT_NOVALNET_CC_CAPTURE_AUTHORIZE', 'capture', '6', '4', 'xtc_cfg_select_option(array(\'capture\', \'authorize\'), ','', now()),

            ('MODULE_PAYMENT_NOVALNET_CC_MANUAL_CHECK_LIMIT', '', '6', '5', '', '', now()),  
            ('MODULE_PAYMENT_NOVALNET_CC_SHOP_TYPE', 'False',  '6', '6', 'xtc_cfg_select_option(array(\'False\', \'ONECLICK\', \'ZEROAMOUNT\'), ','', now()),
    ...
    public function keys()
    {
        ...
        return array (
            ...
            'MODULE_PAYMENT_NOVALNET_CC_ENFORCE_3D',
    ...
    public function getNovalnetCCFormDetails()
    {
        ...
        $iframedata = array(
             ...
             'enforce_3d' => (MODULE_PAYMENT_NOVALNET_CC_ENFORCE_3D == 'True') ? '1' : '0',
             );
