...
class novalnet_ideal extends NovalnetPayment
{
    var $code = 'novalnet_ideal',
        $title,
        $description,
        $enabled,
        $key = 49,
    ...
    public function selection()
    {
        ...
        if (!NovalnetUtil::checkMerchantConfiguration() || !$this->validateAdminConfiguration() || !NovalnetUtil::hidePaymentVisibility(NovalnetUtil::getPaymentAmount((array)$order), MODULE_PAYMENT_NOVALNET_IDEAL_VISIBILITY_BYAMOUNT)) {
            return false;
        }
        ...
        if (MODULE_PAYMENT_NOVALNET_IDEAL_TEST_MODE == 'True') {
            $notification = MODULE_PAYMENT_NOVALNET_TEST_MODE_MSG;
        }

        // Information to the end user
        $notification .= trim(strip_tags(MODULE_PAYMENT_NOVALNET_IDEAL_ENDCUSTOMER_INFO));
    ...
    public function before_process()
    {
		...
		if (isset($post['status']) && $post['status'] == '100') {
                ...
                NovalnetUtil::updateComments(array(
                    'order_status' => NovalnetUtil::checkDefaultOrderStatus(MODULE_PAYMENT_NOVALNET_IDEAL_ORDER_STATUS),
                    'order_no'     => $order_id,
    ...
    public function install()
    {
        if (NovalnetUtil::globalConfigInstallError($this->code)) {
			...
			VALUES
			('MODULE_PAYMENT_NOVALNET_IDEAL_ALLOWED', '', '6', '0', '', '', now()),
			('MODULE_PAYMENT_NOVALNET_IDEAL_STATUS','False', '6', '1', 'xtc_mod_select_option(array(\'True\' => MODULE_PAYMENT_NOVALNET_TRUE,\'False\' => MODULE_PAYMENT_NOVALNET_FALSE),\'MODULE_PAYMENT_NOVALNET_IDEAL_STATUS\',".MODULE_PAYMENT_NOVALNET_IDEAL_STATUS.",' , '',now()),
			('MODULE_PAYMENT_NOVALNET_IDEAL_TEST_MODE','False', '6', '2', 'xtc_mod_select_option(array(\'True\' => MODULE_PAYMENT_NOVALNET_TRUE,\'False\' => MODULE_PAYMENT_NOVALNET_FALSE),\'MODULE_PAYMENT_NOVALNET_IDEAL_TEST_MODE\',".MODULE_PAYMENT_NOVALNET_IDEAL_TEST_MODE.",' , '',now()),
			('MODULE_PAYMENT_NOVALNET_IDEAL_VISIBILITY_BYAMOUNT', '', '6', '3','',  '', now()),
			('MODULE_PAYMENT_NOVALNET_IDEAL_ENDCUSTOMER_INFO', '', '6', '4','',  '', now()),
			('MODULE_PAYMENT_NOVALNET_IDEAL_SORT_ORDER', '0', '6', '5', '',  '', now()),
			('MODULE_PAYMENT_NOVALNET_IDEAL_ORDER_STATUS', '1',  '6', '6', 'xtc_cfg_pull_down_order_statuses(', 'xtc_get_order_status_name', now()),
			('MODULE_PAYMENT_NOVALNET_IDEAL_PAYMENT_ZONE', '0', '6', '7', 'xtc_cfg_pull_down_zone_classes(', 'xtc_get_zone_class_title',now())
			");
        }
    }
    ...
    public function keys()
    {
        ...
        return array(
            ...
            'MODULE_PAYMENT_NOVALNET_IDEAL_VISIBILITY_BYAMOUNT',
    ...
    {
        if (MODULE_PAYMENT_NOVALNET_IDEAL_STATUS == 'True' && defined('MODULE_PAYMENT_NOVALNET_IDEAL_BLOCK_TITLE')) {
            // Validate payment visibility amount
            if (MODULE_PAYMENT_NOVALNET_IDEAL_VISIBILITY_BYAMOUNT != '' && !preg_match('/^\d+$/', MODULE_PAYMENT_NOVALNET_IDEAL_VISIBILITY_BYAMOUNT)) {
                if ($admin) {
                    echo NovalnetUtil::novalnetBackEndShowError(MODULE_PAYMENT_NOVALNET_IDEAL_BLOCK_TITLE);
                }
                return false;
            }
        }
