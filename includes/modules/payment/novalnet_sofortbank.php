...
class novalnet_sofortbank extends NovalnetPayment
{
    var $code = 'novalnet_sofortbank',
        $title,
        $description,
        $enabled,
        $key = 33,
    ...
    public function selection()
    {
        ...
        if (!NovalnetUtil::checkMerchantConfiguration() || !$this->validateAdminConfiguration() || !NovalnetUtil::hidePaymentVisibility(NovalnetUtil::getPaymentAmount((array)$order), MODULE_PAYMENT_NOVALNET_SOFORTBANK_VISIBILITY_BYAMOUNT)) {
            return false;
        }
        ...
        $notification .= trim(strip_tags(MODULE_PAYMENT_NOVALNET_SOFORTBANK_ENDCUSTOMER_INFO));
        $notification = !empty($notification) ? $notification.'<br/>' :'';

		$selection = $this->checkoutSelectionDetails();

        $selection['description'] .= $this->description . MODULE_PAYMENT_NOVALNET_REDIRECT_NOTICE_MSG. $notification;
    ...
    public function before_process()
    {
		...
		if (isset($post['tid'])) {
			...
			$test_mode = (int)(!empty($payment_response['test_mode']) || MODULE_PAYMENT_NOVALNET_SOFORTBANK_TEST_MODE == 'True');
			...
			if (isset($post['status']) && $post['status'] == '100') {
                ...
                NovalnetUtil::updateComments(array(
                    'order_status' => NovalnetUtil::checkDefaultOrderStatus(MODULE_PAYMENT_NOVALNET_SOFORTBANK_ORDER_STATUS),
                    'order_no'     => $order_id,
                    'comments'     => NovalnetUtil::formPaymentComments($payment_response['tid'], $test_mode)
                ));
            } else {
    ...
    public function check()
    {
        if (!isset($this->_check)) {
            $check_query = xtc_db_query("select configuration_value from ".TABLE_CONFIGURATION." where configuration_key = 'MODULE_PAYMENT_NOVALNET_SOFORTBANK_ALLOWED'");
    ...
    public function install()
    {
        if (NovalnetUtil::globalConfigInstallError($this->code)) {
			...
			VALUES
			('MODULE_PAYMENT_NOVALNET_SOFORTBANK_ALLOWED', '', '6', '0', '', '', now()),
			('MODULE_PAYMENT_NOVALNET_SOFORTBANK_STATUS','False', '6', '1', 'xtc_mod_select_option(array(\'True\' => MODULE_PAYMENT_NOVALNET_TRUE,\'False\' => MODULE_PAYMENT_NOVALNET_FALSE),\'MODULE_PAYMENT_NOVALNET_SOFORTBANK_STATUS\',".MODULE_PAYMENT_NOVALNET_SOFORTBANK_STATUS.",' , '',now()),
			('MODULE_PAYMENT_NOVALNET_SOFORTBANK_TEST_MODE','False', '6', '2', 'xtc_mod_select_option(array(\'True\' => MODULE_PAYMENT_NOVALNET_TRUE,\'False\' => MODULE_PAYMENT_NOVALNET_FALSE),\'MODULE_PAYMENT_NOVALNET_SOFORTBANK_TEST_MODE\',".MODULE_PAYMENT_NOVALNET_SOFORTBANK_TEST_MODE.",' , '',now()),
			('MODULE_PAYMENT_NOVALNET_SOFORTBANK_VISIBILITY_BYAMOUNT', '', '6', '3','',  '', now()),
			('MODULE_PAYMENT_NOVALNET_SOFORTBANK_ENDCUSTOMER_INFO', '', '6', '4','',  '', now()),
			('MODULE_PAYMENT_NOVALNET_SOFORTBANK_SORT_ORDER', '0', '6', '5', '',  '', now()),
			('MODULE_PAYMENT_NOVALNET_SOFORTBANK_ORDER_STATUS', '1',  '6', '6', 'xtc_cfg_pull_down_order_statuses(', 'xtc_get_order_status_name', now()),
			('MODULE_PAYMENT_NOVALNET_SOFORTBANK_PAYMENT_ZONE', '0', '6', '7', 'xtc_cfg_pull_down_zone_classes(', 'xtc_get_zone_class_title',now())
			");
        }
    }
    ...
    public function keys()
    {
        ...
        return array(
            'MODULE_PAYMENT_NOVALNET_SOFORTBANK_ALLOWED',
            'MODULE_PAYMENT_NOVALNET_SOFORTBANK_STATUS',
            'MODULE_PAYMENT_NOVALNET_SOFORTBANK_TEST_MODE',
            'MODULE_PAYMENT_NOVALNET_SOFORTBANK_VISIBILITY_BYAMOUNT',
            'MODULE_PAYMENT_NOVALNET_SOFORTBANK_ENDCUSTOMER_INFO',
    ...
    {
        if (MODULE_PAYMENT_NOVALNET_SOFORTBANK_STATUS == 'True' && defined('MODULE_PAYMENT_NOVALNET_SOFORTBANK_BLOCK_TITLE')) {
            // Validate payment visibility amount
            if (MODULE_PAYMENT_NOVALNET_SOFORTBANK_VISIBILITY_BYAMOUNT != '' && !preg_match('/^\d+$/', MODULE_PAYMENT_NOVALNET_SOFORTBANK_VISIBILITY_BYAMOUNT)) {
                if ($admin) {
