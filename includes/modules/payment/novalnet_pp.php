...
class novalnet_pp extends NovalnetPayment
{
    ...
    public function selection()
    {
        ...
        if (MODULE_PAYMENT_NOVALNET_PP_SHOP_TYPE == 'ZEROAMOUNT') {
            $notification .=  MODULE_PAYMENT_NOVALNET_PP_ZERO_AMOUNT_COMMENTS;
        }
        ...
        $selection['description'] .= '<span id="novalnet_pp_description">'.$this->description .MODULE_PAYMENT_NOVALNET_REDIRECT_NOTICE_MSG .'</span>' .$notification.'<script src="'.DIR_WS_CATALOG.'includes/external/novalnet/js/novalnet_pp.js'.'" type="text/javascript"></script>';
        ...
        //pponeclick
            ...
            <input type="hidden" id="nn_lang_pp_redirection_desc" value="' . $this->description . MODULE_PAYMENT_NOVALNET_REDIRECT_NOTICE_MSG . '"/>
            ...
            <p id="nn_old" style="display:none">'.MODULE_PAYMENT_NOVALNET_PP_TEXT_DESCRIPTION. MODULE_PAYMENT_NOVALNET_REDIRECT_NOTICE_MSG.'</p>
            <input type="hidden" id="nn_lang_pp_given_account" value="' . MODULE_PAYMENT_NOVALNET_PP_GIVEN_ACCOUNT . '"/>'
            );
        } else {
				$selection = $this->checkoutSelectionDetails();
				$selection['description'] .= '<span id="novalnet_pp_description">'.$this->description .MODULE_PAYMENT_NOVALNET_REDIRECT_NOTICE_MSG .'</span>' .$notification.'<input type="hidden" name="novalnet_pp_change_account" id="novalnet_pp_change_account" value="0"/>';
        }

        if (MODULE_PAYMENT_NOVALNET_PP_SHOP_TYPE == 'ONECLICK') {
               $selection['fields'][] = array(
                    'title' => '',
                    'field' => '<div id="nn_pp_savecard">'.xtc_draw_checkbox_field('nn_pp_savecard', 1, false, 'id=nn_pp_savecard').MODULE_PAYMENT_NOVALNET_PP_SAVECARD_DETAILS .'</div>'
                                    );
        }
    ...
    public function install()
    {
        if (NovalnetUtil::globalConfigInstallError($this->code)) {
			...
			('MODULE_PAYMENT_NOVALNET_PP_ORDER_STATUS', '1',  '6', '10', 'xtc_cfg_pull_down_order_statuses(', 'xtc_get_order_status_name', now()),
    ...
    public function keys()
    {
        ...
        echo '<input type="hidden" id="nn_root_catalog" value="'. DIR_WS_CATALOG .'" /><input type="hidden" id="nn_pp_message" value="'.MODULE_PAYMENT_NOVALNET_PP_SHOW_MESSAGE.'"/><script src="'.DIR_WS_CATALOG . 'includes/external/novalnet/js/novalnet_pp.js" type="text/javascript"></script> <script src="'.DIR_WS_CATALOG . 'includes/external/novalnet/js/novalnet_auth_cap.js" type="text/javascript"></script>';
