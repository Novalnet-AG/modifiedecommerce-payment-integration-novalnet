...
class novalnet_prepayment extends NovalnetPayment
{
    ...
    public function install()
    {
        if (NovalnetUtil::globalConfigInstallError($this->code)) {
		  ...
		  VALUES
			...
			('MODULE_PAYMENT_NOVALNET_PREPAYMENT_VISIBILITY_BYAMOUNT', '', '6', '3','',  '', now()),
			('MODULE_PAYMENT_NOVALNET_PREPAYMENT_ENDCUSTOMER_INFO', '', '6', '4','',  '', now()),
			('MODULE_PAYMENT_NOVALNET_PREPAYMENT_SORT_ORDER', '0', '6', '5', '',  '', now()),
			('MODULE_PAYMENT_NOVALNET_PREPAYMENT_ORDER_STATUS', '1',  '6', '6', 'xtc_cfg_pull_down_order_statuses(', 'xtc_get_order_status_name', now()),
			('MODULE_PAYMENT_NOVALNET_PREPAYMENT_CALLBACK_ORDER_STATUS', '1',  '6', '7', 'xtc_cfg_pull_down_order_statuses(', 'xtc_get_order_status_name', now()),
			('MODULE_PAYMENT_NOVALNET_PREPAYMENT_PAYMENT_ZONE', '0', '6', '8', 'xtc_cfg_pull_down_zone_classes(', 'xtc_get_zone_class_title',now())
		");
        }
    }
    ...
    public function validateAdminConfiguration($admin = false)
    {
		if (MODULE_PAYMENT_NOVALNET_PREPAYMENT_STATUS == 'True' && defined('MODULE_PAYMENT_NOVALNET_PREPAYMENT_BLOCK_TITLE')) {
            ...
            if (MODULE_PAYMENT_NOVALNET_PREPAYMENT_VISIBILITY_BYAMOUNT != '' && !preg_match('/^\d+$/', MODULE_PAYMENT_NOVALNET_PREPAYMENT_VISIBILITY_BYAMOUNT)) {
                if ($admin) {
