...
class novalnet_invoice extends NovalnetPayment
{
    ...
    function __construct()
    {
        ...
        if (strpos(MODULE_PAYMENT_INSTALLED, $this->code) !== false) {
            if(defined('MODULE_PAYMENT_NOVALNET_INVOICE_ENABLE_FRAUDMODULE')){
    ...
    public function selection()
    {
        ...
        if (!isset($_SESSION['novalnet'][$this->code]['tid']) && $payment_implementation_type == 'guarantee' && empty($order->billing['company']) ) {
    ...
    public function process_button()
    {
        ...
        if (isset($post['novalnet_invoice_new_pin']) && $post['novalnet_invoice_new_pin'] == 1) {
            ...
            $payment_error_return = 'payment_error=' . $this->code . '&error=' . urlencode(trim(NovalnetUtil::setUtf8Mode($response['status_message'])));
			xtc_redirect(xtc_href_link(FILENAME_CHECKOUT_PAYMENT, $payment_error_return, 'SSL', true, false));
            ...
            $payment_error = 'payment_error=' . $this->code . '&error=' . urlencode(MODULE_PAYMENT_NOVALNET_AMOUNT_ERROR_MESSAGE);
        ...
        if (isset($_SESSION['novalnet'][$this->code]['secondcall']) && $_SESSION['novalnet'][$this->code]['secondcall']) {
            ...
            if($payment_implementation_type == 'guarantee' && (!$invalid_valid_dob || $order->billing['company'] != '')) {
                ...
                if(!empty($_SESSION['novalnet'][$this->code]['novalnet_invoicebirthdate'])) {
                    $input_params['birth_date']   = date('Y-m-d',strtotime($_SESSION['novalnet'][$this->code]['novalnet_invoicebirthdate']));
                } else {
            ...
            if ($payment_response['status'] == '100') {
                ...
                // Novalnet transaction status got failure for displaying error message
                $error_msg = 'payment_error=' . $this->code . '&error=' . urlencode(NovalnetUtil::getTransactionMessage($payment_response));
    ...
    public function install()
    {
        if (NovalnetUtil::globalConfigInstallError($this->code)) {
            ...
            ('MODULE_PAYMENT_NOVALNET_INVOICE_STATUS', 'False',  '6', '1', 'xtc_cfg_select_option(array(\'True\', \'False\'), ','', now()),
            ('MODULE_PAYMENT_NOVALNET_INVOICE_TEST_MODE', 'False',  '6', '2', 'xtc_cfg_select_option(array(\'True\', \'False\'), ','', now()),
            ('MODULE_PAYMENT_NOVALNET_INVOICE_CAPTURE_AUTHORIZE', 'capture', '6', '3', 'xtc_cfg_select_option(array(\'capture\', \'authorize\'), ','', now()),('MODULE_PAYMENT_NOVALNET_INVOICE_MANUAL_CHECK_LIMIT', '', '6', '4', '', '', now()),
            ('MODULE_PAYMENT_NOVALNET_INVOICE_ENABLE_FRAUDMODULE', 'False',  '6', '5', 'xtc_cfg_select_option(array(\'True\', \'False\'), ','', now()),
            ...
            ('MODULE_PAYMENT_NOVALNET_INVOICE_ENABLE_GUARANTEE', 'False',  '6', '14', 'xtc_cfg_select_option(array(\'True\', \'False\'), ','', now()),
            ...
            ('MODULE_PAYMENT_NOVALNET_INVOICE_FORCE_NON_GUARANTEE', 'True',  '6', '17', 'xtc_cfg_select_option(array(\'True\', \'False\'), ','', now())          
            ");
