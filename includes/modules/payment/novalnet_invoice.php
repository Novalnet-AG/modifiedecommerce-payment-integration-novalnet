...
$selection['description'] .= '
        ...
        <script src="'.DIR_WS_CATALOG.'includes/external/novalnet/js/novalnet_invoice.js'.'" type="text/javascript"></script>';
    ...
    public function pre_confirmation_check()
    {
        ...
        list($payment_implementation, $error) = NovalnetUtil::paymentImplementationType($order, $this->code);
        ...
        if ($payment_implementation == 'guarantee') {

            $_SESSION['novalnet'][$this->code]['process_guarantee'] = true;

            if (!empty($post['novalnet_invoicebirthdate']) && NovalnetUtil::validateAge($post['novalnet_invoicebirthdate'])) {
            ...
            if (MODULE_PAYMENT_NOVALNET_INVOICE_FORCE_NON_GUARANTEE != 'True' && empty($order->customer['company']) ) {
				if (empty($post['novalnet_invoicebirthdate'])) {					
                ...
                } elseif (NovalnetUtil::validateAge($post['novalnet_invoicebirthdate'])) { // Validate age
                    xtc_redirect(xtc_href_link(FILENAME_CHECKOUT_PAYMENT, 'error_message=' . NovalnetUtil::setUtf8Mode(MODULE_PAYMENT_NOVALNET_AGE_ERROR), 'SSL', true, false));
                }
            } 
        } else {
            ...
            } else { //Validate fraud module field
                $this->fraud_module_status = NovalnetUtil::setFraudModuleStatus($this->code, $this->fraud_module);
    ...
    public function before_process()
    {
        ...
        if (isset($_SESSION['novalnet'][$this->code]['secondcall']) && $_SESSION['novalnet'][$this->code]['secondcall']) {
            ...
            if (!empty($_SESSION['novalnet'][$this->code]['process_guarantee']) && $payment_implementation_type == 'guarantee') { // Appending parameters for guarantee payment
                $input_params['key']          = '41';
                $input_params['payment_type'] = 'GUARANTEED_INVOICE';
                if( empty($order->customer['company']) ) {
					$input_params['birth_date'] = $_SESSION['novalnet'][$this->code]['novalnet_invoicebirthdate'];
				}
            } else {
                $this->fraud_module_status = NovalnetUtil::setFraudModuleStatus($this->code, $this->fraud_module);
    ...
    public function after_process()
    {
        ...
        $order_status = ($_SESSION['novalnet'][$this->code]['process_guarantee'] == '1') ? ($_SESSION['novalnet'][$this->code]['gateway_status'] == '75' ? MODULE_PAYMENT_NOVALNET_INVOICE_GUARANTEE_PENDING_ORDER_STATUS : MODULE_PAYMENT_NOVALNET_INVOICE_CALLBACK_ORDER_STATUS) : ($_SESSION['novalnet'][$this->code]['gateway_status'] == '100' ? MODULE_PAYMENT_NOVALNET_INVOICE_ORDER_STATUS : MODULE_PAYMENT_NOVALNET_ONHOLD_ORDER_COMPLETE);
    ...
    public function install()
    {
        if (NovalnetUtil::globalConfigInstallError($this->code)) {
		  ...
		  VALUES
			...
			('MODULE_PAYMENT_NOVALNET_INVOICE_ORDER_STATUS', '1',  '6', '11', 'xtc_cfg_pull_down_order_statuses(', 'xtc_get_order_status_name', now()),
			('MODULE_PAYMENT_NOVALNET_INVOICE_CALLBACK_ORDER_STATUS', '1',  '6', '12', 'xtc_cfg_pull_down_order_statuses(', 'xtc_get_order_status_name', now()),
			...
			('MODULE_PAYMENT_NOVALNET_INVOICE_GUARANTEE_MIN_AMOUNT', '', '6', '15','',  '', now()),
			('MODULE_PAYMENT_NOVALNET_INVOICE_FORCE_NON_GUARANTEE','True', '6', '16', 'xtc_mod_select_option(array(\'True\' => MODULE_PAYMENT_NOVALNET_TRUE,\'False\' => MODULE_PAYMENT_NOVALNET_FALSE),\'MODULE_PAYMENT_NOVALNET_INVOICE_FORCE_NON_GUARANTEE\',".MODULE_PAYMENT_NOVALNET_INVOICE_FORCE_NON_GUARANTEE.",' , '',now()),
			('MODULE_PAYMENT_NOVALNET_INVOICE_GUARANTEE_PENDING_ORDER_STATUS', '1',  '6', '17', 'xtc_cfg_pull_down_order_statuses(', 'xtc_get_order_status_name', now())
			");
        }
    }
