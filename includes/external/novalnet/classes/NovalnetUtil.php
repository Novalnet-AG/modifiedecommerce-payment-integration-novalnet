...
class NovalnetUtil
{
    ...
    public static function merchantValidate($admin)
    {
        ...
        // Global configuration value pattern.
        $pattern = "/[\w-]+\|(|\d+)\|(|\d+)$/";

        // Global configuration values.
        $value = trim(MODULE_PAYMENT_NOVALNET_PUBLIC_KEY) . '|' . MODULE_PAYMENT_NOVALNET_REFERRER_ID . '|' . MODULE_PAYMENT_NOVALNET_GATEWAY_TIMEOUT;
    ...
    /**
     * Display last successful payment
     * @param $payment
     *
     * @return void
     */
    public static function getLastSuccessPayment($payment)
    {
        if (empty($_SESSION['payment']) && MODULE_PAYMENT_NOVALNET_LAST_SUCCESSFUL_PAYMENT_SELECTION == 'True' && self::getLastSuccessTransPayment($payment) && empty($_SESSION['novalnet'][$payment]['tid'])) {
    ...
    public static function doPaymentCurlCall($paygate_url, $data, $build_query = true)
    {
        ...
        curl_setopt($curl_process, CURLOPT_TIMEOUT, (MODULE_PAYMENT_NOVALNET_GATEWAY_TIMEOUT != '' ? MODULE_PAYMENT_NOVALNET_GATEWAY_TIMEOUT : 240));
    ...
    public static function getCommonRequestParams($data)
    {
        ...
        $urlparam = array(
          ...
          'remote_ip'        => $client_ip,
          'system_ip'        => $system_ip,
          'system_version'   => (! empty ($shop_version['version'] ) ? $shop_version['version']:'').'-NN11.2.0',
        ...
        // Append referrer id
        $additional_parameters['referrer_id'] = trim(MODULE_PAYMENT_NOVALNET_REFERRER_ID);
    ...
    public static function formRedirectionParams(&$redirect_data)
    {
        ...
        $redirect_data['return_url']       = $redirect_data['error_return_url']  = xtc_href_link(FILENAME_CHECKOUT_PROCESS, '', 'SSL');
    ...
    {
        return (!empty($response['status_message']) ? self::setUtf8Mode($response['status_message']) : !empty($response['status_desc']) ? self::setUtf8Mode($response['status_desc']) : (!empty($response['status_text']) ? self::setUtf8Mode($response['status_text']) : MODULE_PAYMENT_NOVALNET_TRANSACTION_ERROR));
    }
    ...
    public static function transactionFailure($response, $payment, $error_message = '')
    {
        ...
        xtc_db_perform(TABLE_ORDERS_STATUS_HISTORY, array (
            ...
            'date_added'        => 'now()',
            'customer_notified' => (int) (SEND_EMAILS == 'true'),
            'comments'          => $message.$transaction_comments,
        ));
     ...
     * @return boolean
     */
    public static function getLastSuccessTransPayment($payment_code)
    {
        if ($_SESSION['account_type'] == 1) {
            return false;
        }
        $query = xtc_db_query("SELECT payment_type FROM novalnet_transaction_detail WHERE customer_id='". xtc_db_input($_SESSION['customer_id']) ."' ORDER BY id DESC LIMIT 1");
        $sqlQuerySet = xtc_db_fetch_array($query);
        ...
        if ($guarantee_payment == 'True') {
			 if ((((int) $amount >= (int) $minimum_amount) && in_array($customer_iso_code, array('DE', 'AT', 'CH')) && $order->info['currency'] == 'EUR' && $delivery_address === $billing_address)) {
                ...
                if (!in_array($customer_iso_code, array('DE', 'AT', 'CH'))) {
                    $guarantee_error .= MODULE_PAYMENT_NOVALNET_FORCE_GUARANTEE_ERROR_MESSAGE_COUNTRY;                  
                } 
                if ($order->info['currency'] !== 'EUR' ) {
                    $guarantee_error .= '<br/>'.MODULE_PAYMENT_NOVALNET_FORCE_GUARANTEE_ERROR_MESSAGE_CURRENCY;                 
                } 
                if ( ! empty( array_diff( $billing_address, $delivery_address ) ) ) {
                    $guarantee_error .= '<br/>'.MODULE_PAYMENT_NOVALNET_FORCE_GUARANTEE_ERROR_MESSAGE_ADDRESS;                  
                } 
                if ( (int) $amount < (int) $minimum_amount ) {
                    $guarantee_error .= '<br/>'.sprintf(MODULE_PAYMENT_NOVALNET_FORCE_GUARANTEE_ERROR_MESSAGE_AMOUNT, str_replace('.', ',', $minimum_amount/100) .' '. $order->info['currency']);
                }               
    ...
    public static function formInvoicePrepaymentComments($data)
    {
        ...
        $trans_comments .= MODULE_PAYMENT_NOVALNET_AMOUNT.': '.xtc_format_price_order($data['amount'], 1, $data['currency']).PHP_EOL;
    ...
    public static function novalnetReferenceComments($order_id, $data)
    {
        ...
        $comments .=  MODULE_PAYMENT_NOVALNET_INVPRE_REF2 .':  BNR-' . (!empty($data['product']) ? $data['product'] : MODULE_PAYMENT_NOVALNET_PRODUCT_ID) . '-' .  $order_id. PHP_EOL;
    ...
    {
        return xtc_draw_input_field($name, (isset($customer_details['customers_dob']) ? $customer_details['customers_dob'] : ''), 'id="'.$name.'" placeholder="'.MODULE_PAYMENT_NOVALNET_GUARANTEE_DOB_FORMAT.'" autocomplete="OFF" maxlength="10" ') . '<br/><span id="'.$name.'-alert" style="color:red;"></span><script>document.getElementById("'.$name.'").onblur=function(){
    ...
    public static function getNovalnetTransDetails($order_no)
    {
        ...
        $sqlQuery = xtc_db_query("SELECT tid, payment_id, payment_type, amount, gateway_status, test_mode, customer_id, zero_transaction, zerotrxndetails, payment_details,payment_ref,callback_amount,total_amount,date FROM novalnet_transaction_detail WHERE order_no='". xtc_db_input($order_no) ."'");
        $transInfo = xtc_db_fetch_array($sqlQuery);

        if (!$transInfo) {
            $sqlQuery = xtc_db_query("SELECT tid, payment_id, payment_type, amount, gateway_status, test_mode, customer_id, zero_transaction, zerotrxndetails, payment_details,callback_amount,total_amount,date FROM novalnet_transaction_detail WHERE order_no='". xtc_db_input($order_no) ."'");
            $transInfo = xtc_db_fetch_array($sqlQuery);
        }
