...
class NovalnetUtil
{
    ...
    { 
        ...
        if ((!defined('MODULE_PAYMENT_NOVALNET_PUBLIC_KEY') || empty(MODULE_PAYMENT_NOVALNET_PUBLIC_KEY))) {
    ...
    public static function sepaDuedate()
    {
        ...
        if ($sepa_due_date_limit != '' && $sepa_due_date_limit <= 14 && $sepa_due_date_limit >= 3)
        {
            return (int)$sepa_due_date_limit;
        }
    }
    ...
    public static function gotoPaymentOnCallback($payment, $fraud_module = null, $fraud_module_status = null)
    {
        if ($fraud_module && $fraud_module_status) {
            ...
            xtc_redirect(xtc_href_link(FILENAME_CHECKOUT_PAYMENT, 'error_message=' . urlencode(trim($error_message)), 'SSL', true, false));
        }
    }
    ...
    public static function getPaymentAmount($data)
    {
        ...
        if (preg_match('/[^\d\.]/', $total)) {
            xtc_redirect(xtc_href_link(FILENAME_CHECKOUT_PAYMENT, self::setUtf8Mode('error_message=' .urlencode(MODULE_PAYMENT_NOVALNET_AMOUNT_ERROR_MESSAGE)), 'SSL', true, false));
        }
    ...
    public static function getCommonRequestParams($data)
    {
        ...
        $urlparam = array(
          ...
          'remote_ip'        => $client_ip,
          'system_ip'        => $system_ip,
          'system_version'   => (! empty ($shop_version['version'] ) ? $shop_version['version']:'').'-NN11.3.1',
    ...
    public static function transactionFailure($response, $payment, $error_message = '')
    {
        ...
        $payment_error = 'payment_error=' . $payment . '&error=' . urlencode($message);
    ...
    public static function globalConfigInstallError($code)
    {
        if (strpos(MODULE_PAYMENT_INSTALLED, 'novalnet_config') === false) {
            ...
            xtc_redirect(xtc_href_link(FILENAME_MODULES, 'set=payment&error='.urlencode(MODULE_PAYMENT_NOVALNET_CONFIG_INSTALL_ERROR).'&module=' . $code));
        }
    }
   ...
   /**
     ...
     * @return string
     */
    public static function sendPaymentNotificationMail($order_no, $message)
    {
