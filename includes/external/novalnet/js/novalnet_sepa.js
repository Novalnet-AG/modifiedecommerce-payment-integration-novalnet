...
jQuery('document').ready(function () {
	...
	var titleTd = $('#nn_sepa_acc, #nn_sepa_mandate, #nn_sepa_savecard, #novalnet_oneclick_sepa_ref, #novalnet_oneclick_sepa_click_new').closest('tr').find('td.title');
    titleTd.css('width', 'auto');
            ...
            if ( selected_payment == 'novalnet_sepa') {
                if (undefined !== jQuery('#novalnet_sepa_iban')) {
                    ...
                    if (iban === '' && change_account == 'newform') {
                        alert(jQuery('#nn_sepa_account_error').val());
                        return false;
                    } 
                }
				if (undefined !== jQuery('#novalnet_sepa_bic')) {
					var bic = jQuery('#novalnet_sepa_bic').val();
					if (change_account == 'newform' && $('#novalnet_sepa_bic_tr').css('display') === 'block' && bic === '') {
						alert(jQuery('#nn_sepa_account_error').val());
						return false;
					}
				}
    ...
    jQuery('#novalnet_sepa_iban').keyup(function (event) {
           ...
           for(var i = 0; i < field.length;i++){
                   ...
                   if(i > 1){
                           if(field.charAt(i).match(/^[A-Za-z0-9]/)){
                                   value += field.charAt(i);
                           }
                   }
           }
