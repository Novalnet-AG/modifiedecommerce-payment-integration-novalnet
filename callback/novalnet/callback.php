    ...
    switch($payment_type_level) {
        ...
        case 2:
            ...
            if ($success_status) {
                ...
                if (in_array($nn_vendor_params['payment_type'], array('INVOICE_CREDIT', 'CASHPAYMENT_CREDIT', 'ONLINE_TRANSFER_CREDIT', 'CREDIT_ENTRY_DE'))) {

					// Check for paid amount
                    if (($nn_trans_history['order_paid_amount']+$nn_trans_history['callback_old_amount']) < $nn_trans_history['order_total_amount']) {
                        ...
                        if ($nn_trans_history['order_total_amount'] <= $total_amount) {
        ...
        case 1:
            if ($success_status) {
				...
				$callback_comments = PHP_EOL . sprintf($comments, $nn_vendor_params['tid_payment'],$formatted_amount, date('Y-m-d'), date('H:i:s'), $nn_vendor_params['tid']) . PHP_EOL;
        ...
        case 0:
			...
			if (in_array($nn_vendor_params['payment_type'], array('PAYPAL','PRZELEWY24')) && $nn_vendor_params['tid_status'] == '100') {
                ...
                if ($nn_trans_history['order_paid_amount'] < $nn_trans_history['order_total_amount']) {
					...
					// Prepare callback comments
                     $callback_comments = PHP_EOL . sprintf(NovalnetUtil::setUtf8Mode(MODULE_PAYMENT_NOVALNET_CALLBACK_UPDATE_COMMENTS), $nn_vendor_params['shop_tid'], $formatted_amount, date('Y-m-d'), date('H:i:s')) . PHP_EOL;
				...
				if ($nn_trans_history['gateway_status'] == '75' && in_array($nn_vendor_params['tid_status'], array('91', '99'))) {
                    ...
                    $callback_comments .= sprintf(MODULE_PAYMENT_NOVALNET_PENDING_TO_ONHOLD, $nn_vendor_params['shop_tid'], date('Y-m-d H:i:s'));
                    ...
                    $callback_comments .= sprintf(MODULE_PAYMENT_NOVALNET_PAYMENT_CONFIRMED, date('Y-m-d H:i:s'));
                }
               if (in_array($nn_trans_history['payment_type'], array('novalnet_invoice')) && in_array($nn_vendor_params['payment_type'], array('INVOICE_START', 'GUARANTEED_INVOICE')) && $nn_vendor_params['tid_status'] == 100) {
				   $invoice_comments .= sprintf(MODULE_PAYMENT_NOVALNET_PAYMENT_CONFIRMED, date('Y-m-d H:i:s'));
                ...
                if ($nn_trans_history['payment_type'] == 'novalnet_sepa' && $nn_vendor_params['payment_type'] == 'GUARANTEED_DIRECT_DEBIT_SEPA' && $nn_vendor_params['tid_status'] == 100) {
                    $sepa_comments = $nn_vendor_script->getTransactionComments($nn_vendor_params);
                }
                ...
                $callback_comments = $callback_comments.$invoice_comments.$sepa_comments;
                 // Update callback comments
...
class NovalnetVendorScript {
    ...
    function getOrderReference() {
		...
		// Get transaction details
		$sql_query = xtc_db_query("select order_no, amount, payment_id, payment_type, gateway_status, callback_amount, payment_details,total_amount from novalnet_transaction_detail where tid = '".xtc_db_input($this->callback_request_params['shop_tid'])."' ");
    ...
    function sendNotifyMail($message) {
        ...
        if (MODULE_PAYMENT_NOVALNET_CALLBACK_MAIL_SEND == 'True' && NovalnetUtil::validateEmail(MODULE_PAYMENT_NOVALNET_CALLBACK_MAIL_TO)) {
            ...
            if ($email_to != '') {
				// Send E-mail
                xtc_php_mail(EMAIL_FROM,STORE_NAME, $email_to , $email_to_name , $email_bcc, '', '', '', '', $email_subject, $message , '');
                echo 'Mail sent!<br>';
            } else {
                echo 'Mail not sent!';
            }
        ...
        $nn_vendor_params = $this->getCaptureParams();
         ...
         if ($nn_vendor_params['payment_type'] == 'TRANSACTION_CANCELLATION' && $order_values['gateway_status'] != '100') {
           ...
           $callback_comments = PHP_EOL.PHP_EOL. sprintf(MODULE_PAYMENT_NOVALNET_PAYMENT_CANCELLED, date('Y-m-d H:i:s'));
            ...
            $cancel_order_status = MODULE_PAYMENT_NOVALNET_ONHOLD_ORDER_CANCELLED;
			NovalnetUtil::sendPaymentNotificationMail($nn_vendor_params['order_no'], sprintf(MODULE_PAYMENT_NOVALNET_TRANS_DEACTIVATED_MESSAGE, date(DATE_FORMAT), date('H:i:s')));
            // Update callback comments
            $this->updateCallbackComments(array(
    ...
    function getTransactionComments($nn_vendor_params) {
        ...
        if ($nn_vendor_params['payment_type'] != 'INVOICE_START') {
