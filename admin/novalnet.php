...
class NovalnetExtension {
    ...
    function show_amount_update () {
        global $order;
        if(($this->data['payment_id'] == 37 && $this->data['gateway_status'] == 99) || (in_array($this->data['payment_id'], array(27,59)) && $this->data['gateway_status'] == 100 && $this->data['callback_amount'] <  $this->data['total_amount'])) {
    ...
    function process_trans_confirm ($request) {
        ...
        if ($data['status'] == '100') {
            ...
            $message .= (($request['trans_status'] == '100') ?  PHP_EOL . $confirm_message : sprintf(MODULE_PAYMENT_NOVALNET_TRANS_DEACTIVATED_MESSAGE, date(DATE_FORMAT), date('H:i:s'))).PHP_EOL;
    ...
    function process_refund ($request) {
        ...
        if ($data['status'] == '100') {
            // Message to be updated
            $message = PHP_EOL.sprintf(NovalnetUtil::setUtf8Mode(MODULE_PAYMENT_NOVALNET_REFUND_PARENT_TID_MSG), $this->data['tid'], xtc_format_price_order(($request['refund_trans_amount']/100), 1, $order->info['currency']));

            // Check for refund TID
            if (!empty($data['tid'])) {
                $message .= sprintf(NovalnetUtil::setUtf8Mode(MODULE_PAYMENT_NOVALNET_REFUND_CHILD_TID_MSG), $data['tid']);
            }
    ...
    function process_amount_update ($request) {
        global $messageStack, $order;
        $checkdate = (! checkdate($request['amount_change_day'], $request['amount_change_month'], $request['amount_change_year']));
        ...
        if (!empty($request['new_amount'])) {
            ...
            if ($input_due_date != '0000-00-00' && strtotime($input_due_date) < strtotime(date('Y-m-d')) && in_array($this->data['payment_type'], array('novalnet_invoice','novalnet_prepayment', 'novalnet_barzahlen'))) {
                ...
                if ($data['status'] == '100') {
                    // Message to be updated
                    $message = ($this->data['payment_id'] == 37) ? sprintf(NovalnetUtil::setUtf8Mode(MODULE_PAYMENT_NOVALNET_SEPA_TRANS_UPDATED_MESSAGE), xtc_format_price_order(($request['new_amount']/100), 1, $order->info['currency']), date(DATE_FORMAT, strtotime(date('Y-m-d'))), date('H:i:s')).PHP_EOL : (($this->data['payment_id'] == 59) ? sprintf(NovalnetUtil::setUtf8Mode(MODULE_PAYMENT_NOVALNET_CASH_PAYMENT_TRANS_UPDATED_MESSAGE), xtc_format_price_order(($request['new_amount']/100), 1, $order->info['currency']), date(DATE_FORMAT, strtotime($input_due_date))).PHP_EOL : sprintf(NovalnetUtil::setUtf8Mode(MODULE_PAYMENT_NOVALNET_TRANS_UPDATED_MESSAGE), xtc_format_price_order(($request['new_amount']/100), 1, $order->info['currency']), date(DATE_FORMAT, strtotime($input_due_date))).PHP_EOL);
                    $param = array(
    ...
    function process_trans_book ($request) {
        ...
        if ($data['status'] == '100') {
            ...
            // Message to be updated
            $message = PHP_EOL.MODULE_PAYMENT_NOVALNET_TRANSACTION_DETAILS . PHP_EOL . MODULE_PAYMENT_NOVALNET_TRANSACTION_ID . $data['tid'] . $test_mode_notification;
